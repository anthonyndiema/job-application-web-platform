...
if(!$user_detl->is_logged_in())
{
	$user_detl->redirect('login.php');
}
//
$curremail=$_SESSION['userSession'];
        
       $stmtzz = $user_detl->runQuery("SELECT * FROM users WHERE email=:email_id");
	$stmtzz->execute(array(":email_id"=>$person));
	$row = $stmtzz->fetch(PDO::FETCH_ASSOC);
       
if(isset($_POST['btncloseaccount']))	{
    $closeresult=  trim($_POST['closeresult']);
    $userid=$row['id'];
    if($closeresult=="closeyes"){
      $stmtzz = $user_detl->runQuery("delete FROM users WHERE id=:user_id");
	$stmtzz->execute(array(":user_id"=>$userid));
	//$row = $stmtzz->fetch(PDO::FETCH_ASSOC);   
        ...
        $msg = "<div class='alert alert-success'>
						<button class='close' data-dismiss='alert'>&times;</button>
						You have successfully canceled closing your account.
						</div>";
        $user_detl->redirect("profile.php ");
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="idawn - close your account">
   <title>idawn - close your account</title>
    ...
    <div id="content">
      <div class="container">
          ...
          <div class="col-sm-9 page-content">
            <div class="inner-box">
              <div class="usearadmin">
                  <h5><a href="#">Welcome back,&nbsp;&nbsp;&nbsp; <?php echo $row['fname']."  ".$row['lname'];  ?></a></h5>
              </div>
            </div>
            <div class="inner-box">
                <div>
		...
		?>
              ...
              <p>Confirm that you would like to close your account?</p>
               <form method="post">
                     <label class="radio-inline">
              <input name="closeresult" value="closeyes" type="radio"> Yes
              </label>
              <label class="radio-inline">
              <input name="closeresult" value="closeno" type="radio"> No
              </label>
              <br>
              <input type="submit" name="btncloseaccount" class="btn btn-common pt" value="Proceed"/>
                </form>
