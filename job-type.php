...
<html lang="en">
  <head>
   ...
   <meta name="keywords" content="idawn,IT,information technoloy,computer science,banking,accounting,CPA,certified,professional,finance,commerce,admin,clerical,health,care,engineering,construction,building,hospitality,transportation,retail,sales,marketing,human resource,government,labour,science,technology,biotechnology,executive,quality assurace,supply chain,grocery,agriculture,farming,electrical,civil,broadcasting,journalism,inventory,installation,maintenance,repair,training,job,contract,volunteer,graduate,program,part time,full time,kenya,nairobi,nakuru,kisumu,mombasa,Salary Scale">
    <meta name="description" content="part-time,full-time and contract <?php echo $_GET['jtype'];if($_GET['jtype']!='Job'){echo " Program";}?>s in kenya and beyond">
   <title>part-time,full-time and contract <?php echo $_GET['jtype'];if($_GET['jtype']!='Job'){echo " Program";} ?>s | idawn. The marketplace for jobs </title>
          ...
          <div class="col-sm-9 page-content">
            ...
            <div class="product-filter">
              ...
              <div class="short-name">
                  <h4>Results for <?php echo $jobtype; ?></h4>
...
<input type="hidden" id="jtype" value='<?php echo $jobtype;?>'/>
...
$page_name="../../../jobs/type/".str_replace(' ','-',$jobtype); //If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
$nume = $dbo->query("select count(adid) from advert where jobtype='str_replace('-',' ',$jobtype)' and status='active'")->fetchColumn();
...
$query=" select * from advert  where jobtype='str_replace('-',' ',$jobtype)' and status='active' order by dtpostd desc  limit $eu, $limit ";
