...
<html lang="en">
  <head>
    ...
    <meta name="keywords" content="idawn,engineering,jobs,contract,hospitality,IT,information technology,Telecommunication,journalism,agriculture,surveyor,Full-time,part-time,Legal,internship,kenya,nairobi,nakuru,mombasa,kisumu,thika,legal,admin,programs,">
    <meta name="description" content="<?php echo "jobs in kenya: ".$_GET["jpermit"].$_GET["jcat"]." jobs, internships, volunteer programs ";if(!empty($loc)){echo "in ".$loc;}?>">
   <title><?php echo "".$_GET["jpermit"]." ".$_GET["jcat"]." jobs, internships, volunteer programs ";if(!empty($loc)){echo "in ".$loc;}?></title>
    ...
    <div class="main-container">
      <div class="container">
        <div class="row">
          ...
          <div id="prev-data" class="col-sm-9 page-content">
...
<input type="hidden" id="loc" value='<?php echo str_replace(" ","-",$loc);?>'/>
<input type="hidden" id="jcat" value='<?php echo str_replace(" ","-",$jcat);?>'/>
<input type="hidden" id="jpermit" value='<?php echo str_replace(" ","-",$jpermit);?>'/>
<input type="hidden" id="jtype" value='<?php echo str_replace(" ","-",$jtype);?>'/>
...
$page_name="../../../../../../jobs/".str_replace(' ','-',$jpermit)."/".str_replace(' ','-',$jcat)."/".str_replace(' ','-',$jtype)."/in/".str_replace(' ','-',$loc); //If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
$loc=($loc=="ky")? "":str_replace('-',' ',$loc);
$jcat=(empty($jcat))? "":str_replace('-',' ',$jcat);
$jtype=(empty($jtype))? "":str_replace('-',' ',$jtype);
$jpermit=(empty($jpermit))? "":str_replace('-',' ',$jpermit);	

if(!empty($loc)&&!empty($jcat)&&!empty($jtype)&&!empty($jpermit)){
$resultsqu="SELECT COUNT(*) FROM advert";
$resultsqu.=" WHERE jlocation like '%".$loc."%'";
if($jcat!="all"&&$jtype!="all"&&$jpermit!="all"){
$resultsqu.=" AND adcategory='".$jcat."' AND jobtype='".$jtype."' AND jobpermit='".$jpermit."'";
}
if($jcat!="all"&&$jtype!="all"&&$jpermit=="all"){
$resultsqu.=" AND adcategory='".$jcat."' AND jobtype='".$jtype."'";
}
if($jcat!="all"&&$jtype=="all"&&$jpermit!="all"){
$resultsqu.=" AND adcategory='".$jcat."' AND jobpermit='".$jpermit."'";
}
if($jcat=="all"&&$jtype!="all"&&$jpermit!="all"){
$resultsqu.=" AND jobtype='".$jtype."' AND jobpermit='".$jpermit."'";
}
if($jcat!="all"&&$jtype=="all"&&$jpermit=="all"){
$resultsqu.=" AND adcategory='".$jcat."'";

}
if($jtype!="all"&&$jcat=="all"&&$jpermit=="all"){
$resultsqu.=" AND jobtype='".$jtype."'";
}
if($jpermit!="all"&&$jtype=="all"&&$jcat=="all"){
$resultsqu.=" AND jobpermit='".$jpermit."'";
}
//$results = $mysqli->query("SELECT COUNT(*) FROM advert where adcategory='".$jcat."' AND jobpermit='".$jpermit."' AND jobtype='".$jtype."' AND jlocation like '%".$loc."%'");
}
else if(empty($loc)&&!empty($jcat)&&!empty($jtype)&&!empty($jpermit)){
$resultsqu="SELECT COUNT(*) FROM advert";
if($jcat!="all"&&$jtype!="all"&&$jpermit!="all"){
$resultsqu.=" WHERE adcategory='".$jcat."' AND jobtype='".$jtype."' AND jobpermit='".$jpermit."'";
}
if($jcat!="all"&&$jtype!="all"&&$jpermit=="all"){
$resultsqu.=" WHERE adcategory='".$jcat."' AND jobtype='".$jtype."'";
}
if($jcat!="all"&&$jtype=="all"&&$jpermit!="all"){
$resultsqu.=" WHERE adcategory='".$jcat."' AND jobpermit='".$jpermit."'";
}
if($jcat=="all"&&$jtype!="all"&&$jpermit!="all"){
$resultsqu.=" WHERE jobtype='".$jtype."' AND jobpermit='".$jpermit."'";
}
if($jcat!="all"&&$jtype=="all"&&$jpermit=="all"){
$resultsqu.=" WHERE adcategory='".$jcat."'";
}
if($jtype!="all"&&$jcat=="all"&&$jpermit=="all"){
$resultsqu.=" WHERE jobtype='".$jtype."'";
}
if($jpermit!="all"&&$jtype=="all"&&$jcat=="all"){
$resultsqu.=" WHERE jobpermit='".$jpermit."'";
}
//$results = $mysqli->query("SELECT COUNT(*) FROM advert where adcategory='".$jcat."' AND jobpermit='".$jpermit."' AND jobtype='".$jtype."'");
}


if($resultsqu=="SELECT COUNT(*) FROM advert"){
$resultsqu.=" WHERE status='active'";
}
if($resultsqu!="SELECT COUNT(*) FROM advert"){
$res2=" AND status='active'";
}
$nume = $dbo->query($resultsqu)->fetchColumn();
/////// The variable nume above will store the total number of records in the table////
/////////// Now let us print the table headers ////////////////
//echo "<TABLE class='t1'>";
//echo  "<tr><th>ID</th><th>Name</th><th>Class</th><th>Mark</th></tr>";
////////////// Now let us start executing the query with variables $eu and $limit  set at the top of the page///////////
$query=str_replace("COUNT(*)","*",$resultsqu)." order by dtpostd desc  limit $eu, $limit ";
?> 
<?php
echo "<div class='item-list'>There are $nume jobs Available</div>";
	foreach ($dbo->query($query) as $row) {
	$link="../../../../../../".str_replace(' ','-',"jobs/".$row['adcategory']."/".$row['adid']."/".$row['adname']);
$delim=",";
$text= $row['skills'];
