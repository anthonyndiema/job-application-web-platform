...
$pending_ads = new USER();
           // All database details will be included here 
if($pending_ads->is_logged_in()=="")
{
	$pending_ads->redirect('login.php');
}
 $person=$_SESSION['userSession']; $curremail=$person;
if(isset($_POST["del-job-btn"])){
    $adid_s=$_POST["adid_s"];
     $stmt = $pending_ads->runQuery("DELETE FROM advert WHERE adid=:adid");
$stmt->execute(array(":adid"=>$adid_s));
$pending_ads->redirect("pending-ads.php");
}
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="pending adverts">
   <title>pending adverts</title>
      ...
      <div class="container">
        <div class="row">
          ...
          <div class="col-sm-9 page-content">
            <div class="inner-box">
              <h2 class="title-2">Adverts Pending Approval</h2>
              <div class="table-responsive">
                 <?Php
require "configure.php";           // All database details will be included here 

$page_name="pending-ads.php"; //If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
$nume = $dbo->query("select count(adid) from advert where useremail='".$person."' and status='pending'")->fetchColumn();
...
$query=" select * from advert  where useremail='".$person."' and status='pending'  limit $eu, $limit ";
...
?>                         
                <table class="table table-striped table-bordered add-manage-table">
                  ...
                  <tbody>
                   ...
                   if($nume==0)
                   {
                       echo '<tr><td colspan="7" style="color:red;"><center>There are no Jobs pending Approval</center></td></tr>';
                   }
                   else{
   ...
   echo "<tr>
                      ...
                      <td class='action-td'>
                        ...
                        <p><a class='btn btn-primary btn-xs' href='preview-ad.php?ad_id=".$row["adid"]."'><i class='fa fa-eye'></i> Preview</a></p>                       
